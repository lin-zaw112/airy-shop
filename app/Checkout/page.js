...
export default function Page() {
  ...
  const uiStatus = notification !== null ? notification.status : "";
  const IsSucceed = uiStatus === "success";
  ...
  return (
    <main className="mx-auto my-12 w-5/6 overflow-hidden px-4 py-10 shadow md:w-2/4 lg:w-1/4">
      ...
      <div className="flex flex-col">
        ...
        <Button
          className={`relative flex h-16 w-full justify-around ${
            IsSucceed && "disabled:bg-green-400"
          }`}
          active
          disabled={!hasItemInCart || IsSucceed}
          handler={CheckoutCart}
        >
          <svg
            className={`absolute ${
              !IsPending ? "hidden" : ""
            } h-6 w-6 animate-spin fill-yellow-400 text-neutral-50`}
            viewBox="0 0 100 101"
            fill="none"
          ...
          {IsSucceed && <CheckIcon className="h-6 w-6" />}
          {!IsPending && !IsSucceed && <span>Place Order</span>}
        </Button>
      </div>
    </main>
  );
